...
import com.edwin.compiler.activity.entity.JavaOnResultListener
...
class StartMethodBuilder(private val activityClass: ActivityClass, private val name: String) {
    ...
    fun build(typeBuilder: TypeSpec.Builder) {
        ...
        val methodBuilderOfView = MethodSpec.methodBuilder(name)
            ...
            .addStatement(
                "Activity activity = \$T.INSTANCE.findProperActivity(view)",
        ...
        if (sharedElements.isNotEmpty()) {
            ...
            methodBuilderOfContext.addStatement("\$T options = null", BUNDLE.java)
                ...
                .addStatement(
                    "\$T sharedElements = new \$T<>()",
                    ARRAY_LIST[PAIR[VIEW, STRING]].java,
                    ARRAY_LIST.java
                )
            ...
            for (sharedElement in sharedElements) {
                if (sharedElement.sourceId == 0) {
                    if (firstNeedTransitionNameMap) {
                        methodBuilderOfContext.addStatement(
                            "\$T nameMap = new \$T<>()",
                            HASH_MAP[STRING, VIEW].java,
                            HASH_MAP.java
                        )
                            .addStatement(
                                "\$T.findNamedViews(activity.getWindow().getDecorView(), nameMap)",
                                VIEW_UTILS.java
                            )
                    ...
                    methodBuilderOfContext.addStatement(
                        "sharedElements.add(new Pair<>(nameMap.get(\$S), \$S))",
                        sharedElement.sourceName,
                        sharedElement.targetName
                    )
                } else {
                    methodBuilderOfContext.addStatement(
                        "sharedElements.add(new Pair<>(activity.findViewById(\$L), \$S))",
            ...
            methodBuilderOfContext.addStatement(
                "options = \$T.makeSceneTransition(context, sharedElements)",
                ACTIVITY_BUILDER.java
            )
                .endControlFlow()
        } else {
        ...
        if (activityClass.hasResult) {
            ...
            methodBuilderOfContext.addStatement(
                "\$T.INSTANCE.startActivityForResult(context, intent, \$L, \$L, \$L, \$L)",
                ACTIVITY_BUILDER.java,
                optionsName,
            ...
            methodBuilderOfContext.addStatement(
                "\$T.INSTANCE.startActivity(context, intent, \$L, \$L, \$L)",
                ACTIVITY_BUILDER.java,
                optionsName,
                pendingTransition.enterAnim,
                pendingTransition.exitAnim
