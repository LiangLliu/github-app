    ...
    open fun build(typeBuilder: TypeSpec.Builder) {
        ...
        if (fragmentClass.sharedElements.isEmpty()) {
            methodBuilder.addStatement(
                "return \$T.showFragment((\$T) activity, \$L, containerId, tag, intent.getExtras(), \$T.class, null)",
                FRAGMENT_BUILDER.java,
            ...
            methodBuilder.addStatement(
                ...
                ARRAY_LIST[PAIR[STRING, STRING]].java,
                ARRAY_LIST.java
            )
            ...
            for (sharedElement in fragmentClass.sharedElements) {
                if (sharedElement.sourceId == 0) {
                    methodBuilder.addStatement(
                        "sharedElements.add(new Pair<>(\$S, \$S))",
                        sharedElement.sourceName,
                        sharedElement.targetName
                    )
                } else {
                    methodBuilder.addStatement(
                        "sharedElements.add(new Pair<>(\$T.getTransitionName(container.findViewById(\$L)), \$S))",
                        VIEW_COMPAT.java,
            ...
            methodBuilder.addStatement(
                "return \$T.showFragment((\$T) activity, \$L, containerId, tag, intent.getExtras(), \$T.class, sharedElements)",
                FRAGMENT_BUILDER.java,
                FRAGMENT_ACTIVITY.java,
                isReplace,
                fragmentClass.typeElement
